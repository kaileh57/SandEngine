...
impl Slider {
    pub fn draw(&self, frame: &mut [u8], _text_renderer: &mut TextRenderer) {
...
impl Button {
    pub fn draw(&self, frame: &mut [u8], text_renderer: &mut TextRenderer) {
        ...
        let text_offset = if let Some(color) = self.color {
            ...
            // Draw a border around the color sample
            let border_color = if self.is_selected { C_UI_HIGHLIGHT } else { C_UI_BUTTON_BORDER };
            
            for x in color_box_x..color_box_x + color_box_size {
                let top_idx = (color_box_y * WINDOW_WIDTH as usize + x) * 4;
                let bottom_idx = ((color_box_y + color_box_size - 1) * WINDOW_WIDTH as usize + x) * 4;
                
                if top_idx + 3 < frame.len() {
                    frame[top_idx..top_idx+4].copy_from_slice(&border_color);
                }
                if bottom_idx + 3 < frame.len() {
                    frame[bottom_idx..bottom_idx+4].copy_from_slice(&border_color);
                }
            }
            
            for y in color_box_y..color_box_y + color_box_size {
                let left_idx = (y * WINDOW_WIDTH as usize + color_box_x) * 4;
                let right_idx = (y * WINDOW_WIDTH as usize + color_box_x + color_box_size - 1) * 4;
                
                if left_idx + 3 < frame.len() {
                    frame[left_idx..left_idx+4].copy_from_slice(&border_color);
                }
                if right_idx + 3 < frame.len() {
                    frame[right_idx..right_idx+4].copy_from_slice(&border_color);
                }
            }
            
            color_box_size + 8 // Offset for text
        } else {
            0
        };
        ...
        text_renderer.draw_text(
            frame,
            self.rect.x + text_offset + 8, // Offset text from color sample or edge
            self.rect.y + self.rect.height / 2 - 6,
            &self.label,
            16.0,
            C_UI_TEXT,
        );
        ...
        // Draw button border
        let border_color = if self.action == ButtonAction::ClearSimulation {
            C_UI_CLEAR_BUTTON_BORDER
        } else {
            C_UI_BUTTON_BORDER
        };
        
        self.draw_border(frame, border_color);
    }
    ...
    pub fn draw(&self, frame: &mut [u8], text_renderer: &mut TextRenderer) {
        ...
        for y in self.rect.y..self.rect.y + self.rect.height {
            for x in self.rect.x..self.rect.x + self.rect.width {
                let idx = (y * WINDOW_WIDTH as usize + x) * 4;
                if idx + 3 < frame.len() {
                    frame[idx..idx+4].copy_from_slice(&[30, 30, 35, 255]);
                }
            }
        }
        
        // Draw status items
        let mut offset_x = self.rect.x + 10;
        for (key, value) in &self.items {
            // Draw key with lighter color
            text_renderer.draw_text(
                frame,
                offset_x,
                self.rect.y + self.rect.height / 2 - 6,
            ...
            offset_x += key.len() * 8 + 5; // Approximate width of text
            
            // Draw value with different color
            text_renderer.draw_text(
                frame,
                offset_x,
                self.rect.y + self.rect.height / 2 - 6,
            ...
            offset_x += value.len() * 8 + 20; // Spacing between items
        }
    }
} 